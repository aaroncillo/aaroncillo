...
local config = {
    uppos = {x=597,y=299,z=7},
    dowpos = {x=603,y=310,z=7},
    playerpos = {x=605,y=307,z=7},
    block_start={x=600,y=301,z=7},
    exit={x=605,y=304,z=7},
    delay=10,
    item=1739,
    effects = {
        mark=3,
...
local function pedra(pos,cid,area) -- Function by mock
        ...
        if cre_pos.x == config.playerpos.x+1 then
            ...
            if checkItensInArea(1486,pos, area[config.data[1]])  or checkItensInArea(config.item,pos, area[config.data[1]]) then
                pos.x = pos.x-1
        ...
        elseif cre_pos.x == config.playerpos.x and cre_pos.y == config.playerpos.y-1 then
            ...
            if #area > 1 then
                ...
                if checkItensInArea(1486,pos, area[rot]) or checkItensInArea(config.item,pos, area[rot]) then
                    doSendAnimatedText(cre_pos, "Block", math.random(1,255))
                else
                    config.data[1] = rot
                end
        ...
        pos.stackpos=1
        if getThingfromPos(pos).itemid == config.item then
            doPlayerSendTextMessage(config.data[2], 24, 'tu perdiste!')
        ...
        local humrox = pos
        pos.y = pos.y+1
        if not checkItensInArea(1486,pos, area[config.data[1]]) and not checkItensInArea(config.item,humrox, area[config.data[1]],uida) then
