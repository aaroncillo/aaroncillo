...
function onSay(cid, words, param)
	...
	if (words == "!seepoll") then
		if (getPlayerGroupId(cid) < Poll.CONST_POLL_ADM) then
			...
			if (_sp ~= nil) then
				doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_BLUE, "Mostrando la duración de 10 encuestas de.")
				_spn = 0
		...
		if (getPlayerGroupId(cid) < Poll.CONST_POLL_ADM) then
			...
			if (param == "") or (not isNumber(param)) or (param < 1) then
				...
				if (Poll:exists(param)) then
					...
					doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_BLUE, "Votos a favor de esta encuesta de identificación no se ha encontrado.")
				end
			end
			return TRUE
		end
