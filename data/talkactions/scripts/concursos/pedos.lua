...
local evtpos = {x=436, y=588, z=7}
	...
	elseif words == '/pedos' then
		if param == 'on' then
			if getPlayerAccess(cid) >= 3 then
				setGlobalStorageValue(75310, 9)
                                     doTeleportThing(cid,evtpos)
									 doBroadcastMessage(' '.. getCreatureName(cid) ..' Ha ACTivado el concurso de: pedos para entrar di: !pedos', MESSAGE_EVENT_ADVANCE)
                else
                        doPlayerSendCancel(cid,'You not have access for use this command.')
                end
...
doBroadcastMessage(' '.. getCreatureName(cid) ..' Ha DESactivado el concurso de: pedos', MESSAGE_EVENT_ADVANCE)
                else
                        doPlayerSendCancel(cid,'You not have access for use this command.')
                        end
end
