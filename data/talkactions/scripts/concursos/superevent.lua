...
local evtpos = {x=694, y=584, z=7}
 
	if words == '!sevent' then
	...
	elseif words == '/superevent' then
		if param == 'on' then
			if getPlayerAccess(cid) >= 3 then
				setGlobalStorageValue(75310, 9)
                                     doTeleportThing(cid,evtpos)
									 doBroadcastMessage(' '.. getCreatureName(cid) ..' Ha ACTivado el concurso de: superevent para entrar di: !sevent', MESSAGE_EVENT_ADVANCE)
                else
                        doPlayerSendCancel(cid,'You not have access for use this command.')
                end
...
doBroadcastMessage(' '.. getCreatureName(cid) ..' Ha DESactivado el concurso de: super event', MESSAGE_EVENT_ADVANCE)
                else
                        doPlayerSendCancel(cid,'You not have access for use this command.')
                        end
end
