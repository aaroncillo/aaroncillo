...
local evtpos = {x=1002, y=531, z=7}
	...
	elseif words == '/carrera' then
		if param == 'on' then
			if getPlayerAccess(cid) >= 3 then
				setGlobalStorageValue(75310, 9)
                                     doTeleportThing(cid,evtpos)
									 doBroadcastMessage(' '.. getCreatureName(cid) ..' Ha ACTivado el concurso de: carrera para entrar di: !carrera', MESSAGE_EVENT_ADVANCE)
                else
                        doPlayerSendCancel(cid,'You not have access for use this command.')
                end
...
doBroadcastMessage(' '.. getCreatureName(cid) ..' Ha DESactivado el concurso de: carrera', MESSAGE_EVENT_ADVANCE)
                else
                        doPlayerSendCancel(cid,'You not have access for use this command.')
                        end
end
