...
if getPlayerStorageValue(player, 12346) < 0 then
    if((doCreatureSetNoMove(player, 1))== LUA_ERROR) then
        ...
        doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_BLUE, "El player no se puede mover.")
        doSendAnimatedText(getCreaturePosition(player), "congelado D:", 64)
...
elseif getPlayerStorageValue(player, 12346) >= 0 then
    if((doCreatureSetNoMove(player, 0))== LUA_ERROR) then
        ...
        setPlayerStorageValue(player, 12346, -1)
        doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_BLUE, "El player se puede mover nuevamente.")
        doSendAnimatedText(getCreaturePosition(player), "liberado :D", 168)
