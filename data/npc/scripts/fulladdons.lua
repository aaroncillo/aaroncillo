...
npcHandler:setMessage(MESSAGE_GREET, "Ola |PLAYERNAME|. tu me puedes ayudar? si me ayudas, yo te recompenso con lindos addons! tienes que decir {addons} o {help} si usted no sabe que hacer (los items se compran con bulma en este subterraneo vip) osea por alla --->.")
...
function playerBuyAddonNPC(cid, message, keywords, parameters, node)
    ...
    if (parameters.confirm ~= true) and (parameters.decline ~= true) then
        if(getPlayerPremiumDays(cid) == 0) and (parameters.premium == true) then
            npcHandler:say('Disculpe, este addon es para jugadores premium!', cid)
            npcHandler:resetNpc()
            return true
        end
        if (getPlayerStorageValue(cid, parameters.storageID) ~= -1) then
            npcHandler:say('usted ya tiene este addon!', cid)
            npcHandler:resetNpc()
            return true
        end
    ...
    elseif (parameters.confirm == true) then
        ...
        if(getPlayerMoney(cid) >= addoninfo.cost) and (items_number == table.maxn(addoninfo.items)) then
            ...
            npcHandler:say('usted no tiene los items necesarios o el dinero!', cid)
        end
        npcHandler:resetNpc()
        return true
    elseif (parameters.decline == true) then
        npcHandler:say('Este no le interesa? Experimente otro!', cid)
        npcHandler:resetNpc()
        return true
    end
    return false
